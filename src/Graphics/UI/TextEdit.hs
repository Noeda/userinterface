...
newtype TextEdit = TextEdit (ManagedQObject QTextEdit)
                   deriving ( Eq, Typeable
                            , HasQObject
                            , Touchable )

instance IsWidget TextEdit where
    getWidget = coerceManagedQObject . getManagedQObject

instance HasManagedQObject TextEdit QTextEdit where
    getManagedQObject (TextEdit man) = man

instance CentralWidgetable TextEdit

-- | Creates a text edit. Initially it is editable by the user.
createTextEdit :: UIAction () -> UIAction TextEdit
...
htmlContent :: TextEdit -> UIVar' Text
